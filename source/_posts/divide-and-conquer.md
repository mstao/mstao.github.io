...
“分”和“治”的过程是十分简单明了的，但“合”的过程相当于另一个算法，就是如何将两个有序数组合并为一个数组，这个在leetcode是一个problem，地址为：[88. 合并两个有序数组](https://leetcode-cn.com/problems/merge-sorted-array)，有兴趣的可以去尝试一下。这里给出利用一个temp数组来做数据中转，主要思路是：首先比较两个数组的元素，谁小就把谁先放到temp数组中，比较完毕后，两个数组可能有一个数组中的数据还没有移动到temp数组（两个数组长度不一致），不过剩下的部分不要担心，我们直接将其拷贝放到temp数组的后面就可以了，因为这部分的值必然是大于temp数组中已有的值的。整个排序算法的实现如下：
